...
public:
    ...
    void Initialize(const Eigen::VectorXcd& _origVc,
        ...
        currEdgeDeviations.resize(EV.rows());
        ...
        for (int i=0;i<xSize/2;i++){
            ...
            complexJValues(closeTriOffset+i)=closeFactor;
        }
        ...
        if (exactMC || exactIAP){  //deviation constraints are included
            ...
            for (int i=0;i<EV.rows();i++){
                ...
                complexGradCols(deviationTriOffset+5*i+2)=origVc.rows()+EV(i,0);
        ...
        for (int i=0;i<complexJRows.size();i++){
            ...
            //real lower right
            JRows(2*i+2*complexGradRows.size()+1)=ComplexRowOffset+complexJRows(i);
    ...
    void update_energy(const Eigen::VectorXd& x){
        ...
        for (int i=0;i<EV.rows();i++)
            AMAPVec(i)=(CurrMobRecips(E2V(i,0))*(OrigVc(E2V(i,1))-OrigVc(E2V(i,0)))*CurrMobRecips(E2V(i,1))-(CurrLocations(E2V(i,1))-CurrLocations(E2V(i,0))));
    ...
    void update_constraints(const Eigen::VectorXd& x)
    {
        ...
        if (!isExactMC && !isExactIAP){
            ...
            for (int i=0;i<EV.rows();i++)
                deviationVec(i)=currY(EV(i,0))*(origVc(EV(i,1))-origVc(EV(i,0)))*currY(E2V(i,1))-currE(i)*currEdges(i);
    ...
    void update_gradient(const Eigen::VectorXd& x){
        ...
        for (int i=0;i<EV.rows();i++){
            ...
            //Yj derivative
            complexGradValues(rigidTriOffset+2*i+1)=smoothFactor*yigidRatio;
        }
    ...
    void pre_iteration(const Eigen::VectorXd& prevx){
        initSolution.array().real()=prevx.head(CurrSolutionReal.size()/2);
        initSolution.array().imag()=prevx.tail(CurrSolutionReal.size()/2);
    ...
    bool post_iteration(const Eigen::VectorXd& x){
        ...
        initSolution.array().real()=x.head(CurrSolutionReal.size()/2);
        initSolution.array().imag()=x.tail(CurrSolutionReal.size()/2);
        update_constraints(x);
